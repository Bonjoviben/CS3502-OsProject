 ...
 * This is the CPU class for the Jequirity Operating System.
 */
public class CPU implements Runnable {
     ...
     */
    private ArrayList<Integer> VMA = new ArrayList<Integer>();
    ...
    {
        String binInstr = Helpers.convertFromHexStringToBinaryString(instruction.substring(2));
        String tmpInstr = binInstr;
        instType=Integer.parseInt(tmpInstr.substring(0,2));
        opCode=Helpers.convertFromBinaryStringToDecimalInteger(tmpInstr.substring(2, 8));
        ...
        switch(instType)
        {
            case 00:
            {
                ...
                tmpSReg1=Helpers.convertFromBinaryStringToDecimalInteger(tmpInstr.substring(8,12));
                tmpSReg2=Helpers.convertFromBinaryStringToDecimalInteger(tmpInstr.substring(12, 16));
                tmpDstReg=Helpers.convertFromBinaryStringToDecimalInteger(tmpInstr.substring(16, 20));
                break;
            }
            case 01:
            {
                //conditional jump
                tmpBReg=Helpers.convertFromBinaryStringToDecimalInteger(tmpInstr.substring(8, 12));
                tmpDstReg=Helpers.convertFromBinaryStringToDecimalInteger(tmpInstr.substring(12, 16));
                tmpAddress=Helpers.convertFromBinaryStringToDecimalInteger(tmpInstr.substring(16));
                break;
            }
            case 10:
            {
                //unconditional jump
                tmpAddress=Helpers.convertFromBinaryStringToDecimalInteger(tmpInstr.substring(8));
                break;
            }
            case 11:
            {
                //IO operation
                tmpReg1=Helpers.convertFromBinaryStringToDecimalInteger(tmpInstr.substring(8, 12));
                tmpReg2=Helpers.convertFromBinaryStringToDecimalInteger(tmpInstr.substring(12, 16));
                tmpAddress=Helpers.convertFromBinaryStringToDecimalInteger(tmpInstr.substring(16));
                break;
            }
            default:
    ...
    public void executeInstruction(int instruction)
    {
        ...
        switch(opcode)
        {
            case 0: //0 RD
            {
                ...
                if(tmpReg2 > 0)
                {
                    registers[tmpReg1] = Helpers.convertFromHexToDecimal(cache[registers[tmpReg2]/4].substring(2));
//                    log(String.format("Reading val:%s from R%s into R%s", registers[tmpReg1], tmpReg2, tmpReg1 ));
                ...
                else
                {
                    ...
                    registers[tmpReg1] = Helpers.convertFromHexToDecimal( cache[tmpAddress/4].substring(2));
...
//                    log(String.format("WR writing from R%s val:%s to R%s val: %s", tmpReg1, registers[tmpReg1], tmpReg2, registers[tmpReg2]));
                ...
                {
                    cache[tmpAddress/4] = "0x" + Helpers.convertFromDecimalToHex(registers[tmpReg1]);
...
//                    log(String.format("WR writing from R%s val: %s to cache[%s] val:%s ", tmpReg1, registers[tmpReg1], tmpAddress/4, cache[tmpAddress/4] ));
            ...
            case 2: //2 ST
            {
               cache[registers[tmpDstReg]/4] = "0x" + Helpers.convertFromDecimalToHex( registers[tmpBReg]);
//                log(String.format("ST storing from R%s val:%s into cache[%s] val:%s", tmpBReg, registers[tmpBReg], registers[tmpDstReg]/4, cache[registers[tmpDstReg]/4] ));
            ...
            case 3: //LW
            {
                registers[tmpDstReg]= Helpers.convertFromHexToDecimal(cache[(registers[tmpBReg]/4) + tmpAddress].substring(2));
...
//                    log(String.format("BLZ Does NOT branch to address %s because R%s val:%s > 0", tmpAddress/4, tmpBReg, registers[tmpBReg]));
    ...
    public void load(PCB job){
        ...
        System.out.println("cpu.loading job: " + job.getJobNumber());
       ...
       // int instructionCounter = 1;
        ...
        setJobLoaded(true);
        System.out.println("cpu.loading job: " + job.getJobNumber() + " complete");
    ...
    public void unload(PCB current)
    {
        ...
        System.out.println("UNLOADING JOB " + currentJobNumber() + "PROGRAM COUNTER:" + programCounter + " JOB CNTR:" + jobCounter);
